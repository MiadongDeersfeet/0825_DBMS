...
/* ↓ Left Pading / Right Pading
 * LPAD / RPAD (STR, 최종적으로 반환할 문자의 길이(바이트), 패딩할 문자)
 * : 인자로 전달한 문자열에 임의의 문자를 왼쪽 또는 오른쪽에 덧붙여서 N길이만큼의 문자열을 반환해주는 함수
 * 
 * 결과값은 CHARACTER타입으로 반환합니다. (오라클에서 문자를 반환해주는 타입)
 * 덧붙이고자 하는 문자는 생략 가능
 */
...
SELECT
	   LPAD(EMAIL, 25) -- 이메일 값 왼 쪽에 공백이 생겼네? 공백문자가 1바이트인데 25바이트만큼 빵빵하게 채웠다.
  FROM 
  	   EMPLOYEE;	
 ...
 * TRIM
 * 
 * - TRIM(BOTH / LEADING / TRAILRING '제거하고자 하는 문자' FROM STR)
...
SELECT 
	   EMAIL
	 , REPLACE(EMAIL, 'kh.or.kr', 'iei.co.kr') -- 실제 데이터값이 변하는 건 아니고 조회했을 때 보여지는 값이 바뀌는 것입니다.
  FROM
  	   EMPLOYEE;
---------------------------- 문자열 함수 끝!!! --------------------------------------------
...
SELECT
	   FLOOR(123.45 * 10) / 10 -- 역시 수학을 포기하지 말았어야 해.
  FROM 
	   DUAL;
...
-- 각 직원별로 고용일로부터 오늘까지의 근무 일수를 조회 + 이름조회을 어제보다 예쁘게 해봅시다.
SELECT
	   EMP_NAME
  ...
  FROM
	   EMPLOYEE
 WHERE
	   FLOOR(SYSDATE - HIRE_DATE) > 365 * 17; -- 역시 수학을 포기하지 말았어야 해.

-----------------------------------------------------------------------------------------
/*
 * TRUNC
 * - TRUNC(NUMBER, 위치) : 위치지정이 가능한 절삭처리 함수
 * 숫자 123.456을 소수점 아래 둘째 자리까지 남기고 버림(truncate) 합니다
 */
 ...
 * TIMESTAMP <- 얘는 나중에
 * 일반적으로는 DATE 타입 씁니다. TIMESTAMP 는 나노초까지 나오기 때문에 금융거래 시간을 찍는다거나 더 정밀한 작업이 필요할 때 씁니다.
 */
...
-- MONTHS_BETWEEN(DATE1, DATE2) : 두 날짜 사이의 개월 수 차이를 반환(NUMBER)
-- EMPLOYEE 테이블로부터 각 사원의 사원명, 고용일로부터 근무일수
 ...
 */
--SELECT
--	     TRUNC(ADD_MONTHS(SYSDATE, 4)) AS "4개월 후"
--  FROM
--	     DUAL;
...
-- 언어변경 ALTER SESSION SET NLS_LANGUAGE = AMERICAN;
...
----------------------------------------------------------------------------------------------------------------
/* ☆★☆★☆★☆★ 활용도 높음
 * EXTRACT : 년도 또는 월 또는 일 정보를 추출해서 반환(NUMBER 타입)
 * 
...
SELECT
	   EMP_NAME
	 , TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)) AS "년도"
...
SELECT
	   1234
	 , TO_CHAR(1234)
	 , TO_CHAR(1234, '00000') -- 포맷 방식인데 01234 로 나오네? 자리수보다 큰 공간을 0으로 채움
	 , TO_CHAR(1234, '99999') -- 자리수보다 큰 공간을 공백문자로 채움
	 , TO_CHAR(1234, '00000L') -- 설정된 나라(LOCAL)의 화폐단위 / 뒤에 붙이면 뒤에 나오네
	 , TO_CHAR(1234, '$99999')
...
SELECT
	   SYSDATE -- 2025-08-26 11:19:29.000
	 , TO_CHAR(SYSDATE)  -- 25/08/26
	 , TO_CHAR(SYSDATE, 'YYYY-MM-DD') -- 포맷 변경
	 , TO_CHAR(SYSDATE, 'PM HH:MI:SS') -- PM 오전 / 오후 출력
	 , TO_CHAR(SYSDATE, 'HH24:MI:SS') -- 24시간 표기형식
	 , TO_CHAR(SYSDATE, 'MON DY, YYYY') -- 8월 화, 2025 라고 나오네???!
  FROM
	   DUAL;
------------------------------------------------------------------------------------------------------------------------------
-- 년도로 쓸 수 있는 포맷
SELECT
	   TO_CHAR(SYSDATE, 'YYYY') || '년' AS "연도"
...
SELECT
	   TO_CHAR(SYSDATE, 'DD')
	 , TO_CHAR(SYSDATE, 'D') -- 3이라고 나오는데 일요일부터 세기 / 일요일(1), 월요일(2), 화요일(3) / 오늘은 화요일
	 , TO_CHAR(SYSDATE, 'DDD') -- 238 이라고 나오네. 1월 1일로부터 238일 지난 시점이라는 뜻
  FROM 
	   DUAL;
-- DD : 한달기준
-- D : 일주일 기준(일요일부터)
...
SELECT
  ...
  FROM 
	   DUAL;
-- 앞에 00이 있으면 날아가서 사실상 1212 밖에 없기 때문에 오류가 납니다. 그래서 홑따옴표로 묶어주면 됩니다.
-- 980607은 1998년으로 인식하네요. 'YYMMDD' 이걸 넣어주니 2098년이 되네요.
...
SELECT
	   EMP_NAME
	 , (SALARY + SALARY * BONUS) * 12 -- 보너스가 null 인 사람은 연봉 출력이 null 로 나온다.
  FROM
	   EMPLOYEE;
...
-- NVL2(컬럼명, 결과값1, 결과값2)
-- 해당 컬럼에 값이 존재할 경우 결과값 1을 반환하고
-- 해당 컬럼에 값이 null값일 경우 결과값 2를 반환한다.
 ...
 * < 선택함수 > 
 * 
 * DECODE(비교대상)(컬럼명/산술연산/함수식), 조건값1, 결과값1, 조건값2, 결과값2, .........);
 *  
 * - 자바에서의 switch문과 유사
 * switch(비교대상) {
 * case 조건값1 : 결과값1;
...
------------------------------ 7행에 써있는데 여기까지 단일함수였습니다~!!! 짝짝짝 사실 이거 안중요함 ---------------------------------------
...
-- KH 직원들의 급여 합계는 얼마인가?
-- 이 때 필요한 게 그룹 함수입니다.
-- 계산기를 칠 수는 없잖아요~! 23명이면 솔직히 누를만 해.
...
SELECT 
  ...
  FROM
	   EMPLOYEE;
-- LENGTH(SALARY)는 23행, SUM(SALARY)는 1행이라서 ResultSet 이 다르기에 함께 사용하면 실행되지 않습니다.
-- 단일행 함수와 그룹함수는 함께 사용할 수 없습니다. : 결과 행의 개수가 다르기 때문
----------------------------------------------------------------------------------------------
...
-- 보너스를 받는 사원의 수
-- COUNT(칼럼명) : 제시한 컬럼값이 NULL이 아닌 행만 개수를 세서 반환
SELECT
	   COUNT(BONUS)
  FROM 
	   EMPLOYEE;
...
-- 현재 사원들이 속해있는 부서 개수를 세려면 중복을 없애줘야하니까 DISTINCT 까지!
