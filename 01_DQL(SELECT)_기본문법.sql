 ...
 */
-- 여러분들이 KH 그룹의 사원이라고 가정해봅시다. EMPLOYEE 라는 표에 사원의 정보를 담아놨는데 조회를 해봅시다!
-- EMPLOYEE 테이블에서 모든 컬럼을 전부 다 조회하겠다.
SELECT * FROM EMPLOYEE; -- 이 방법은 사용해선 안됨
-- * 사용 시 성능에 영향을 끼침
-- 키워드(갈색)는 왼쪽, 구분은 오른쪽. 가운데에 공을 떨어뜨렸을 때 밑으로 걸리는 거 없이 쏘옥 빠지도록.
SELECT  
	   EMP_NAME
	 , EMAIL
	 , SALARY
...
/* SELECT emp_name, email, salary FROM employee; */ -- 이런 형식은 권장하지 않습니다.

-- 명령어, 키워드, 테이블명, 컬럼명은 대/소문자를 가리지않음
-- 소문자로 작성을 해도 잘 작동을 하지만 대문자로 작성하는 습관을 들이는 것이 좋다
...
-- EMPLOYEE 테이블로부터 직원명, 월급, 연봉(== 월급 X 12)
-- 물론 이런다고 테이블에 있는 값이 변하지 않습니다. SELECT는 조회하는 기능이기 때문이지요.
SELECT
       EMP_NAME
     , SALARY
     , SALARY * 12
...
-- 결과값은 일 단위 --> 값이 지저분한 이유는 DATE 안에 시/분/초가 포함되어있으므로
-- 시/분/초까지 연산을 수행해서 지저분하게 나온다.
------------------------------------------------------------
...
SELECT
	   EMP_NAME AS 사원명
	 , SALARY AS "급여(월)" -- 괄호 때문에 컴퓨터가 연산으로 오해하니까 쌍따옴표를 붙여주세요.  
  FROM 
       EMPLOYEE;
...
-- ※ WHERE 문을 쓸 때는 항상 실행순서를 생각해야합니다.
...
-- 3. EMPLOYEE 테이블에서 현재 재직 중인 사원(ENT_YN == 'N')들의 이름, 입사일 조회
 ...
 */
-- EMPLOYEE 테이블에서 부서코드가 'D9'이면서 급여가 500만원 이하인 사원들의
-- 사원명, 부서코드, 급여 조회
SELECT
	   EMP_NAME
	 , DEPT_CODE
...
-- EMPLOYEE 테이블에서 급여가 350 미만이거나 500을 초과하는 사원들의 사번, 이름, 직급코드
SELECT
	   EMP_ID
	 , EMP_NAME
	 , JOB_CODE
...
-- 입사일이 '90/01/01' ~ '03/01/01'인 사원들의 이름, 입사일을 조회
SELECT
	   EMP_NAME
	 , HIRE_DATE
  FROM
 ...
 * 
 * - 특정패턴 --> 와일드카드
 * '%', '_' 두가지를 가지고 패턴을 만들 수 있음
 * 
 * '%' : 0글자 이상
 *       비교대상컬럼 LIKE 'A%' => 컬럼값 중에서 'A'로 시작하는 것만 조회 -> Apple, Add, A
 * 		 비교대상컬럼 LIKE '%A' => 컬럼값 중에서 'A'로 끝나는 것만 조회 -> BananA, GolilA, A
 * 		 비교대상컬럼 LIKE '%A%' => 컬럼값 중에서 'A'가 포함되는 것을 전부 조회
 * '_' : 1글자
 *       비교대상컬럼 LIKE '_A' => 컬럼값의 'A'앞에 무조건 1글자가 있어야만 패턴에 만족
 *       비교대상컬럼 LIKE '__A' => 컬럼값의 'A'앞에 무조건 2글자가 있어야만 패턴에 만족
 */
...
-- EMPLOYEE 테이블에서 성이 '박'씨인 사원들의 사원명, 전화번호 조회
...
SELECT 
  ...
  FROM 
	   EMPLOYEE
 WHERE
	   PHONE LIKE '___9%'; -- 헷갈렸다. 나중에 다시 분석해보자.
-------------------------------------------------------------
SELECT
   	   EMP_NAME
	 , HIRE_DATE
...
SELECT
  ...
  FROM
	   EMPLOYEE
 WHERE
	   -- BONUS IS NULL; -- 컬럼 = NULL 이 아니라 IS NULL 이라는 점!
	   BONUS IS NOT NULL;
---------------------------------------------------------------------------------------------------------------------
/*
 * < 연결연산자 || >
 ...
 * 
 * System.out.println(num + "dkssudgktpdy"); <= 마치 이런 느낌이라는 뜻
 */
SELECT
	   EMP_ID || EMP_NAME 
  FROM 
...
SELECT
	   EMP_ID || '번 사원' || EMP_NAME || '님의 핸드폰 번호는 ' || PHONE || '입니다.' AS "사원의 정보"
  FROM
	   EMPLOYEE;	   
--------------------------------------------------------------------------------------------------------------------------
/*
 ...
 * 3. 연결연산자
 * 4. 비교연산자
 * 5. IS NULL, LIKE, IN (우선순위가 똑같기 때문에 왼쪽부터 처리합니다.)
 * 6. BETWEEN AND
 * 7. NOT
 * 8. AND
 * 9. OR
...
/*
 * < ☆★☆★☆★☆★☆★☆★ ORDER BY 절 ☆★☆★☆★☆★☆★☆★ >
 * 정렬을 별도로 해주지 않으면 ResultSet은 원래 정렬이 되지 않은 상태로 결과가 나옵니다. (우리는 오름차순이 되어있는데 운이 좋은 겁니다. 언제든지 뒤죽박죽이 될 수 있어요.)
 * 정렬용도로 사용하는 구문
...
SELECT
 ...
 ORDER
    BY
   -- 연봉; -- 왜 where 조건문에서는 연봉; 안되고 정렬문에서는 가능할까? 아마 실행순서 때문인 것으로 보는 게 합리적일 것이다.
      2; -- 이건 2번 컬럼을 기준으로 정렬하겠다는 의미인데 권장하지 않는 방법 ※※비추천※※
