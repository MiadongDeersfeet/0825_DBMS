 ...
 * 
 * -- 회원번호, 사원번호, 게시글번호, 예약번호, 주문번호 등등 채번
 * 
 * 1. 시퀀스 객체 생성 구문
 * 
 * [ 표현법 ]
 ...
 * 
 *  * CACHE : 미리 발생할 값을 생성해서 저장을 해둘건지 말건지 (CACHE는 RAM보다는 훨씬 더 고급 메모리!!! 라고 생각하시면 됩니다.)
 * 			  값이 필요할 때마다 매 번 새롭게 값을 생성해내는 것 보다는
 *            캐시공간에 미리 생성된 값들을 가져다 쓰는 것이 훨씬 속도적인 측면에서 이득이 있음
...
-- 수정 : ALTER --> START WITH는 변경 불가능 너무 바꾸고 싶으면 DROP 하고 다시 CREATE
-- 삭제 : DROP
 ...
 * 
 * * 권한 부여(GRANT)
 * - 시스템 권한 : DB에 접근하는 권한, 객체를 생성할 수 있는 권한 ※ 실질적으로 중요한 권한은 "시스템 권한"
 ...
 * 
 * * 시스템 권한은 종류
 * - CREATE SESSION : 계정에 접속할 수 있는 권한(= DB에 접속할 권한이 없음)
 * - CREATE TABLE : 테이블을 생성할 수 있는 권한
 * - CREATE VIEW : 뷰를 생성할 수 있는 권한
 * - CREATE SEQUENCE : 시퀀스를 생성할 수 있는 권한
 * 
 * GRANT 권한1, 권한2, ... TO 계정명; -- 권한 부여 방법
 */
...
SELECT
	   *
  FROM
       ROLE_SYS_PRIVS; -- 열어보면 RESOURCE 랑 CONNECT 에 특정권한들을 집합으로 모아놨네? / GRANT 때 리소스랑 커넥트 주면 권한들 다 부여가능!!!!!!!!
 WHERE
       ROLE IN ()
