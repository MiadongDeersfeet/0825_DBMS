 ...
 * (데이터를 보관하고자 하면 반드시 테이블이 존재해야한다는 의미!!)
 *
 *  [ 표현법 ]
 * 
 ...
 * 					주로 '들어올 값의 글자수'가 정해져 있을 경우
 * 					예) 성별 M/F
 * 
 * - VARCHAR2(바이트크기) : VAR는 '가변'을 의미, 2는 '2배'를 의미
 * 						최대 4000BYTE까지 지정 가능
 * 						가변길이(적은값이 들어오면 맞춰서 크기가 줄어듬)
 * 						--> CLOB, BLOB -- 4000BYTE로 모자를 때, 아마 쓸 일 없을 듯.
 * 
 * - NVARCHAR2(N) : 가변길이, 선언방식(N) --> N : 최대 문자 수 / 괄호 안에 4000 이라고 쓰면 4000글자까지! 가 된다. / 고대히브리어, 이모지 등 깨질 염려가 있는 글자들 ; 다국어지원
 * 					다국어 지원 아주 화끈함 완전 지원
...
--> 회원의 정보를 담는 테이블(아이디, 비밀번호, 이름, 회원가입일)을 담기 위한 테이블 만들기
--> 키워드는 식별자로 사용할 수 없음. (※유저를 꼭 쓰고싶다. 그럼 접두사 TB_ 를 붙여주세요. 물론 권장하는 바는 아님※)
...
INSERT INTO TB_USER VALUES('kth89', 'pass01', '김태호', '25/08/27');
INSERT INTO TB_USER VALUES('psh96', 'pass02', '박수현', '25/08/27');
INSERT INTO TB_USER VALUES('cjy01', 'pass03', '최준영', '25/08/27');
...
-- 아이디와 비밀번호 컬럼에는 NULL값이 들어가선 안된다!
INSERT INTO TB_USER VALUES('whoami', 'idontknow', '아무개', SYSDATE);
INSERT INTO TB_USER VALUES('whoami', 'uknow', '홍길동', SYSDATE);
-- 중복된 아이디값은 존재해선 안된다!
 ...
 * - 제약조건을 부여하면 데이터를 INSERT / UPDATE 할 때마다 제약조건에 위배되지 않는지 검사합니다.
 * 
...
/*
 * 1. NOT NULL 제약조건
 * 해당 컬럼에 반드시 값이 존재해야할 경우 사용(NULL값을 막는다)
...
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KYK03"."TB_USER_NOT_NULL"."USER_NO") 안에 삽입할 수 없습니다
-- Error position: line: 152 pos: 36
...
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KYK03"."TB_USER_NOT_NULL"."USER_ID") 안에 삽입할 수 없습니다
-- Error position: line: 162 pos: 75
...
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KYK03"."TB_USER_NOT_NULL"."USER_PWD") 안에 삽입할 수 없습니다
-- Error position: line: 177 pos: 92
...
VALUES
     ...
     , NULL
     , NULL
       ); -- NOT NULL 제약조건이 달린 컬럼에는 반드시 NULL이 아닌 값이 존재해야한다!!
...
INSERT INTO TB_USER_UNIQUE VALUES (1, 'admin', '1234', '관리자', '남');
INSERT INTO TB_USER_UNIQUE VALUES (2, 'user01', '1234', '홍길동', '남');
INSERT INTO TB_USER_UNIQUE VALUES (3, 'user01', '1234', '둘리', '남');
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KYK03.SYS_C0042404)에 위배됩니다

DROP TABLE TB_USER_UNIQUE;
-- 테이블 삭제 후 다시

INSERT INTO TB_USER_UNIQUE VALUES(1, 'admin', '1234', NULL, NULL);
INSERT INTO TB_USER_UNIQUE VALUES(1, 'admin', '5678', NULL, NULL);
-- 제약조건명을 달아두면 문제가 생긴 원인은 조금 더 쉽게 유추할 수 있다. /223행 참고/ --
...
CREATE TABLE TB_USER_CHECK(
	...
	USER_PWD VARCHAR2(20) NOT NULL,
	GENDER CHAR(3) CHECK(GENDER IN ('남', '여')) -- TRUE일 때만 INSERT 가능
);

INSERT INTO TB_USER_CHECK VALUES(1, 'admin', '1234', '여');
INSERT INTO TB_USER_CHECK VALUES(2, 'user01', 'pass01', '밥');
INSERT INTO TB_USER_CHECK VALUES(3, 'user02', 'pass02', NULL);
 ...
 * 		6. 회원 성별을 저장할 컬럼 : 'M', 'F'만 INSERT 될 수 있음
 * 		7. 전화번호를 저장할 컬럼
 * 		8. 이메일을 저장할 컬럼
 * 		9. 주소를 저장할 컬럼
...
-- 특정 컬럼에 기본값을 설정하는 방법 ===> ※제약조건은 아닙니다※
 ...
 * -> 행들을 구분하는 식별자의 역할
 * 
 * 똑같이 아이스아메리카노를 사면 누가 주문했는지 구분할 수가 없으니까 주문번호를 달아놓잖아요.
 * 예 ) 학생번호, 주문번호, 상품번호, 게시글번호, 사원번호
...
INSERT INTO TB_USER_PK VALUES (1, 'admin', '1234', NULL, NULL);

SELECT * FROM TB_USER_PK;

INSERT INTO TB_USER_PK VALUES (1, 'user01', '132123', NULL, NULL);
-- 기본키 컬럼은 중복값을 INSERT 할 수 없다!!! 프라이머리 키 달아놓았음

INSERT INTO TB_USER_PK VALUES (NULL, 'user02', '35123', NULL, NULL);
-- 기본키 컬럼은 NULL값을 INSERT 할 수 없다.

CREATE TABLE TB_PRIMARYKEY(
	NO NUMBER PRIMARY KEY,
	ID CHAR(2) PRIMARY KEY
);
-- 하나의 테이블에는 "하나의 PRIMARY KEY"만 가질 수 있습니다. 
-- 두 개의 컬럼을 묶어서 하나의 PRIMARY KEY로 만들 수 있음 --> 복합키☆
DROP TABLE VIDEO;
CREATE TABLE VIDEO(
	NAME VARCHAR2(15),
	PRODUCT VARCHAR2(20),
	PRIMARY KEY(NAME, PRODUCT) -- 컬럼 두 개를 묶어서 PRIMARY KEY 하나로 설정 = 복합키
);

INSERT INTO VIDEO VALUES('김윤기', '자바');
INSERT INTO VIDEO VALUES('김윤기', 'DB');
 ...
 * => FOREIGN KEY 제약조건을 이용해서 다른 테이블 간의 관계를 형성할 수 있음
 * 
 * 
 * [ 표현법 ]
 * - 컬럼 레벨 방식
...
CREATE TABLE TB_USER_CHILD ( -- 자식테이블
	...
	-- FOREIGN KEY(GRADE_ID) REFERENCES USER_GRADE 테이블 레벨 방식 (번거로움)
);
...
INSERT INTO TB_USER_CHILD VALUES(5, 'user05', 'pass05', 'G4'); -- 부모테이블에 존재하지 않는 값

-- < Quize > TB_USER_CHILD에 존재하는 모든 회원에 대해서
--   		 회원번호, 회원아이디, 등급명(예: 회장님)을 조회해주세요!

SELECT * FROM TB_USER_CHILD; -- USER_NO	  USER_ID	USER_PWD	GRADE_ID
SELECT * FROM USER_GRADE;    -- GRADE_CODE    GRADE_NAME
...
-- 자식 레코드가 발견되었습니다. // 댓글이 달려있으면 게시글을 삭제할 수 없다던지~~~!
...
-- 1) ON DELETE SET NULL == 부모 데이터 삭제 시 자식레코드도 NULL값 변경
...
CREATE TABLE JOB_COPY
    AS SELECT * FROM JOB;
-- 컬럼들 조회결과의 데이터값들 제약조건 NOT NULL만 복사가 됨
...
-- 1_2) 컬럼수정(MODIFY)
-- 자료형 수정 : ALTER TABLE 테이블명 MODIFY 컬렴명 바꿀데이터타입;
-- DEFAULT 값 수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT 기본값;



/* ALTER USER 계정명 IDENTIFIED BY 바꾸고싶은비밀번호; -- 비밀번호 변경 방법 / 근데 여러분은 바꿀 수 없어요. 권한이 없거든요ㅋ_ㅋ */
-- JOB_CODE 컬럼 데이터 타입을 CHAR(3)으로 바꾸고 싶다면? -- 직급이 늘어나서 J10 을 써야할 수도 있잖아.
...
ALTER TABLE JOB_COPY2 DROP COLUMN JOB_NAME; -- 이거 안돼요. 최소한 1개의 컬럼은 있어야합니다.
 ...
 * - FOREIGN KEY : ADD FOREIGN KEY(컬럼명) REFERENCES
 * - CHECK : ADD CHECK(컬럼명);
 * - UNIQUE : ADD UNIQUE(컬럼명);
 * 
 * - NOT NULL : ※예외※ MODIFY 컬럼명 NOT NULL;
 * 
 * 제약조건 달려있는 걸 삭제
...
-- 제약조건 추가 ※ 제약조건명(JOB_UQ)을 달아주어야 나중에 삭제할 때 편하다.
...
ALTER TABLE JOB_COPY
RENAME COLUMN JOB_DECO TO JOB_CODE;

SELECT * FROM JOB_COPY;

ALTER TABLE JOB_COPY
RENAME COLUMN LOCATION TO LNAME;

-- 3_2) 테이블명 변경 : ALTER TABLE 테이블명 RENAME 기존테이블명 TO 바꿀테이블명
ALTER TABLE JOB_COPY2 RENAME TO JOB_COPY3; -- 기존테이블명 건너뛰고 바로 TO 바꿀테이블명으로 가는 편
